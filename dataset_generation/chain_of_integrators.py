...
frequencies = [omega / (2 * np.pi * OSR)
               for OSR in [1 << exponent for exponent in range(2)]]
phase = 0.0
offset = 0.0
    ...
    for rho in Rho:
        ...
        for amp in amplitudes:
            for freq in frequencies:
                ...
                base_file_name = f"data/chain_of_integrators.N={n}_beta={beta}_rho={rho}_kappa={kappa}_amplitude={amp}_frequency={freq}_phase={phase}_offset={offset}.cbs"

                key = (str(n), str(beta), str(rho), str(kappa), 'sin', str(amp), str(freq), str(phase), str(offset))
