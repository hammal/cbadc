...
class FIRFilter(DigitalEstimator):
    ...
    def write_C_header(self, filename: str):
        ...
        with open(filename, 'w') as f:
            f.write(
                "// Autogenerated using the https://github.com/hammal/cbadc framework" + os.linesep)
            ...
            f.write("// Filter coefficient's h where" + os.linesep)
            f.write("// h[L][M][K3]" + os.linesep)
            # self.h.shape -> (L, K3, M)
            f.write(
                f"double h[{self.analog_system.L}][{self.analog_system.M}][{self.K3}] = ")
            f.write("{")
            for l in range(self.analog_system.L):
                f.write("{")
                for m in range(self.analog_system.M):
                    ...
                    for k in range(self.K3 - 1):
                        f.write(f"{self.h[l,k,m]:.16f},")
                    f.write(f"{self.h[l,-1,m]:.16f}" + "}")
