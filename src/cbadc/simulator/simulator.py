...
class NGSpiceSimulator(Iterator[np.ndarray]):
    ...
    def __init__(
        ...
        size = int(
            np.ceil(t_end / testbench.Xaf.analog_frontend.digital_control.clock.T)
        )
        ...
        self.state_vector = np.zeros(
            (size, testbench.Xaf.analog_frontend.analog_system.N)
        )
        self.time_vector = (
            np.arange(size) * testbench.Xaf.analog_frontend.digital_control.clock.T
        )
    ...
    def get_sampled_state_trajectories(self):
        headers = ['t'] + [
            f'x_{i+1}'
            for i in range(self.testbench.Xaf.analog_frontend.analog_system.N)
        ]
        return headers, np.hstack((self.time_vector, self.state_vector))
    ...
    def parse(self):
        ...
        for s in zip(clk, *s_raw, *state_raw):
            if not trigger_enable and s[0] <= vcm:
                trigger_enable = True
                if index < self.control_vector.shape[0]:
                    self.control_vector[index, :] = float_to_ternary(
                        s[1 : self.testbench.Xaf.analog_frontend.analog_system.M + 1]
                    )
                    self.state_vector[index, :] = s[
                        1
                        + self.testbench.Xaf.analog_frontend.analog_system.M : 1
                        + self.testbench.Xaf.analog_frontend.analog_system.M
                        + self.testbench.Xaf.analog_frontend.analog_system.N
                    ]
                    index += 1
