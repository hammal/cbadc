...
class HadamardPCB:
    ...
    def __init__(self, pcb="B"):
        ...
        A_chain_of_integrators = np.diag(
            np.ones(3), k=-1
        ) - self.r_1 / self.r_4 * np.eye(4, k=0)
        A = (
            1
            / (self.r_1 * self.c_1)
            * np.matmul(-H_n, np.matmul(A_chain_of_integrators, -H_n * buffer_loss))
        )
        B = -1 / (self.r_1 * self.c_1) * np.ones((4, 1))
        CT = np.eye(4)
        # assuming inputs +/-1 from the digital control
        Gamma = -self.v_ctrl / (self.r_2 * self.c_1) * np.hstack((-H_n, np.eye(4)))
    ...
    def simulation_ramp_1_B(self):
        ...
        control_signal_hardware = cbadc.utilities.byte_stream_2_control_signal(
            cbadc.utilities.read_byte_stream_from_url(
                'https://people.ee.ethz.ch/~merik/ramp_20mHz_4.5Vpp_1MHz_100s_pcbB.adc',
        ...
        control_signal_software_simulated = cbadc.utilities.byte_stream_2_control_signal(
            cbadc.utilities.read_byte_stream_from_url(
                'https://people.ee.ethz.ch/~merik/ideal_ramp_20mHz_4.5Vpp_1MHz_100s_pcbB.adc',
                self.analog_system.M,
            ),
            self.analog_system.M,
        )
        return (
            control_signal_hardware,
            control_signal_software_simulated,
            ideal_simulator,
            size,
        )
